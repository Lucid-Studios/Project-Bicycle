...
public sealed class BikeInMotionTests
{
    ...
    public void Bike_In_Motion_Build_Posture_Is_Versioned_As_Calibration_Event()
    {
        ...
        Assert.Contains(notes, note => note.Contains("V1.3.14 records BikeInMotionTests", StringComparison.Ordinal));
        Assert.Contains(notes, note => note.Contains("engram certification review lane", StringComparison.Ordinal));
        Assert.Contains(notes, note => note.Contains("Context-Quarantined Scholarly Review Predicate Substrate", StringComparison.Ordinal));
        Assert.Contains(notes, note => note.Contains("coherence pressure does not create warrant", StringComparison.Ordinal));
    }
