...
public interface ISliLispLlmInterconnectReadinessService
{
    SliLispLlmInterconnectReadinessReceipt Run(
        SliLispLlmInterconnectReadinessRequest request,
        DateTimeOffset timestampUtc);
}

public sealed class DefaultSliLispLlmInterconnectReadinessService : ISliLispLlmInterconnectReadinessService
{
    ...
    public SliLispLlmInterconnectReadinessReceipt Run(
        SliLispLlmInterconnectReadinessRequest request,
        DateTimeOffset timestampUtc)
    {
        ...
        if (request.RequestsForbiddenMotion)
        {
            return CreateReceipt(
                normalized,
                SliLispLlmInterconnectReadinessDisposition.Refused,
                "sli-lisp-llm-interconnect-runtime-motion-refused",
                runtimePath,
                orderedModules,
                ...
                standardError: "Bounded SLI.Lisp LLM interconnect readiness refused because arbitrary eval, action, activation, model binding, provider call, authority grant, executor arm, GEL admission, SelfGEL mutation, heartbeat activation, Actual activation, or continuity admission was requested.",
                timestampUtc);
        }

        var tempRoot = Path.Combine(Path.GetTempPath(), "project-bicycle-sli-llm-ready", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var completed = result.ExitCode == 0 && IsTrue(telemetry, "llm-interconnect-readiness-completed");

            return CreateReceipt(
                normalized,
                completed
                    ? SliLispLlmInterconnectReadinessDisposition.CompletedCold
                    : SliLispLlmInterconnectReadinessDisposition.Withheld,
                completed
                    ? "sli-lisp-llm-interconnect-readiness-completed-cold"
                    : "sli-lisp-llm-interconnect-readiness-withheld",
                runtimePath,
                orderedModules,
                telemetry,
    ...
    private static SliLispLlmInterconnectReadinessReceipt CreateReceipt(
        NormalizedLlmInterconnectReadinessRequest request,
        ...
        new(
            ReceiptHandle: CreateHandle(
                "sli-lisp-llm-interconnect-ready://",
                outcomeCode,
                request.OperatorId,
                request.Domain,
                request.Role,
                ...
                request.AgentEngineIdleReceiptHandle,
                request.ThoughtForm,
            ...
            AgentEngineIdleReceiptHandle: ReadValueOrFallback(telemetry, "source.agent-engine-idle-receipt", request.AgentEngineIdleReceiptHandle),
            ...
            LlmInterconnectReadinessCompleted: IsTrue(telemetry, "llm-interconnect-readiness-completed"),
            InterconnectState: ReadValueOrFallback(telemetry, "llm-interconnect.state", string.Empty),
            ProviderNeutral: IsTrue(telemetry, "llm-interconnect.provider-neutral"),
            ReadyForAdapter: IsTrue(telemetry, "llm-interconnect.ready-for-adapter"),
            ModelAdapterPresent: IsTrue(telemetry, "llm-interconnect.model-adapter-present"),
            ...
            AgentEngineIdleReady: IsTrue(telemetry, "lane.agent-engine-idle.ready"),
            ...
            EngineLlmMayArticulate: IsTrue(telemetry, "engine-llm-seat.may-articulate"),
            EngineLlmMayRehearse: IsTrue(telemetry, "engine-llm-seat.may-rehearse"),
            EngineLlmMayFormCandidates: IsTrue(telemetry, "engine-llm-seat.may-form-candidates"),
            EngineLlmMayBindModel: IsTrue(telemetry, "engine-llm-seat.may-bind-model"),
            EngineLlmMayCallProvider: IsTrue(telemetry, "engine-llm-seat.may-call-provider"),
            EngineLlmMayGrantAuthority: IsTrue(telemetry, "engine-llm-seat.may-grant-authority"),
            EngineLlmMayExecuteAction: IsTrue(telemetry, "engine-llm-seat.may-execute-action"),
    ...
    private static string WriteRunScript(
        ...
        NormalizedLlmInterconnectReadinessRequest request)
    {
        var scriptPath = Path.Combine(tempRoot, "run-llm-interconnect-readiness.lisp");
        var builder = new StringBuilder();

        builder.AppendLine("(format t \"SAN-SLI-LLM-INTERCONNECT-READY-BEGIN~%\")");
        ...
        builder.AppendLine("(unless (and (find-package :sli-core) (fboundp 'sli-core::run-llm-interconnect-readiness)) (error \"bounded LLM interconnect readiness entrypoint missing\"))");
        builder.Append("(let ((result (sli-core::run-llm-interconnect-readiness ");
        ...
        builder.Append(' ');
        builder.Append(ToLispString(request.AgentEngineIdleReceiptHandle));
        builder.Append(' ');
        builder.Append(ToLispString(request.ThoughtForm));
        builder.AppendLine(")))");
        builder.AppendLine("  (format t \"SAN-SLI-LLM-INTERCONNECT-READY-OK~%\")");
    ...
    private static ProcessResult RunSbcl(string runtimePath, string scriptPath, TimeSpan timeout)
    {
        ...
        if (!process.WaitForExit(timeout))
        {
            ...
            return new ProcessResult(-1, standardOutput.GetAwaiter().GetResult(), "SBCL timed out while running bounded LLM interconnect readiness.");
        }
    ...
    private static NormalizedLlmInterconnectReadinessRequest NormalizeRequest(SliLispLlmInterconnectReadinessRequest request) =>
        new(
            ...
            NormalizeValue(request.SessionId, "llm-interconnect-readiness-session"),
            ...
            NormalizeValue(request.AgentEngineIdleReceiptHandle, "agent-engine-idle-receipt-missing"),
            NormalizeValue(request.ThoughtForm, "idle LLM interconnect readiness"));
    ...
    private sealed record NormalizedLlmInterconnectReadinessRequest(
        string OperatorId,
        string Domain,
        string Role,
        string JobClass,
