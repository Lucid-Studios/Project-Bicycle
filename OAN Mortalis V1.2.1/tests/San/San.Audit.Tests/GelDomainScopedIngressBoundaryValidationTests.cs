...
public sealed class GelDomainScopedIngressBoundaryValidationTests
{
    ...
    public void Build_Posture_Records_Gel_Domain_Scoped_Ingress_As_V1315_Cell()
    {
        ...
        Assert.Contains(notes, note => note.Contains("V1.3.15 opens cme.gel-domain-scoped-ingress-boundary", StringComparison.Ordinal));
        Assert.Contains(notes, note => note.Contains("formed candidate substrate may be scoped to a lawful domain", StringComparison.Ordinal));
        Assert.Contains(notes, note => note.Contains("certification review as admission", StringComparison.Ordinal));
    }
