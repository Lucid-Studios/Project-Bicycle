...
$script:ResolvedRepoRoot = [System.IO.Path]::GetFullPath((Split-Path -Parent $PSScriptRoot))
...
function Get-LocalCorpusPath {
    ...
    $localConfigPath = Join-Path (Get-RepoRoot) ".local\private_corpus_root.txt"
...
Write-Host "PASS: No tracked managed files in the Project Bicycle standalone tool surface contain external absolute paths."
exit 0
