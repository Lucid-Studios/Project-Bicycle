...
public interface ISliLispTypedWarmUseRehearsalService
{
    SliLispTypedWarmUseRehearsalReceipt Run(
        SliLispTypedWarmUseRehearsalRequest request,
        DateTimeOffset timestampUtc);
}

public sealed class DefaultSliLispTypedWarmUseRehearsalService : ISliLispTypedWarmUseRehearsalService
{
        ...
        DateTimeOffset timestampUtc)
    {
        ...
        if (request.RequestsForbiddenMotion)
        {
            return CreateReceipt(
                normalized,
                SliLispTypedWarmUseRehearsalDisposition.Refused,
                "sli-lisp-typed-warm-use-runtime-motion-refused",
                runtimePath,
                orderedModules,
                ...
                standardError: "Bounded SLI.Lisp typed warm-use rehearsal refused because arbitrary eval, action, activation, or model binding was requested.",
                timestampUtc);
        }

        var tempRoot = Path.Combine(Path.GetTempPath(), "project-bicycle-sli-warm-use", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var completed = result.ExitCode == 0 && IsTrue(telemetry, "typed-warm-use-rehearsal-completed");

            return CreateReceipt(
                normalized,
                completed
                    ? SliLispTypedWarmUseRehearsalDisposition.CompletedCold
                    : SliLispTypedWarmUseRehearsalDisposition.Withheld,
                completed
                    ? "sli-lisp-typed-warm-use-rehearsal-completed-cold"
                    : "sli-lisp-typed-warm-use-rehearsal-withheld",
                runtimePath,
                orderedModules,
                telemetry,
    ...
    private static SliLispTypedWarmUseRehearsalReceipt CreateReceipt(
        NormalizedTypedWarmUseRequest request,
        ...
        DateTimeOffset timestampUtc)
    {
        ...
        return new SliLispTypedWarmUseRehearsalReceipt(
            ReceiptHandle: CreateHandle(
                "sli-lisp-typed-warm-use://",
                outcomeCode,
                request.OperatorId,
                request.Domain,
                request.Role,
            ...
            TypedWarmUseRehearsalCompleted: IsTrue(telemetry, "typed-warm-use-rehearsal-completed"),
            TypedScopeAccepted: IsTrue(telemetry, "typed-scope.accepted"),
            LiveIngressAcceptedCold: IsTrue(telemetry, "live-ingress.accepted-cold"),
            ...
            TurnLineageReceiptOnly: IsTrue(telemetry, "turn-lineage.receipt-only"),
            SessionLedgerAppendOnly: IsTrue(telemetry, "session-ledger.append-only"),
    ...
    private static string WriteRunScript(
        ...
        NormalizedTypedWarmUseRequest request)
    {
        var scriptPath = Path.Combine(tempRoot, "run-typed-warm-use-rehearsal.lisp");
        var builder = new StringBuilder();

        builder.AppendLine("(format t \"SAN-SLI-TYPED-WARM-USE-BEGIN~%\")");
        ...
        builder.AppendLine("(unless (and (find-package :sli-core) (fboundp 'sli-core::run-typed-warm-use-rehearsal)) (error \"bounded typed warm-use entrypoint missing\"))");
        builder.Append("(let ((result (sli-core::run-typed-warm-use-rehearsal ");
        ...
        builder.AppendLine(")))");
        builder.AppendLine("  (format t \"SAN-SLI-TYPED-WARM-USE-OK~%\")");
        ...
        TimeSpan timeout)
    {
        ...
        if (!process.WaitForExit(timeout))
        {
            ...
            return new ProcessResult(
                ...
                StandardError: "SBCL timed out while running bounded typed warm-use rehearsal.");
        }
    ...
    private static NormalizedTypedWarmUseRequest NormalizeRequest(SliLispTypedWarmUseRehearsalRequest request) =>
        new(
            ...
            NormalizeValue(request.ThoughtForm, "idle typed warm-use rehearsal"));
    ...
    private sealed record NormalizedTypedWarmUseRequest(
        string OperatorId,
        string Domain,
        string Role,
        string JobClass,
